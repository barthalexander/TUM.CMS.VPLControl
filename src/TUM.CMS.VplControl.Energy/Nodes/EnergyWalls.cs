...
namespace TUM.CMS.VplControl.Energy.Nodes
{
    public class EnergyWalls : Node
    {
        ...
        public EnergyWalls(Core.VplControl hostCanvas) : base(hostCanvas)
        {
            ...
            AddOutputPortToNode("text", typeof(List<double>));//List with walls thermal transmittance
            ...
            textBox_l.TextChanged += textBox_TextChanged;//chose to load walls' properties whenever this textbox's value is changed...
            AddControlToNode(textBox_l);//#1
            var label_Ra = new Label { Content = "Rse" };
            AddControlToNode(label_Ra); //#2
            ...
            comboBox_Rse.SelectionChanged += selection_changed;
            AddControlToNode(comboBox_Rse);  //#3
            //label for Uj total thermal transm for all the wall elements
            var label_Uj = new Label { Content = "Uj = " };
            AddControlToNode(label_Uj);     //#4
            var button_Uj = new Button { Content = "Calculate the heat transfer coefficient of Walls" };
        ...
        public /*override*/ void Calculate1()
        {
            ...
            WallsThermalT = new List<double> { };
            Console.WriteLine("Total of " + ifcWallThickness.Count + " wall elements.");
            for (int i = 0; i < ifcWallThickness.Count; i++)
            {
                double denominator = Rsi + ifcWallThickness[i] / l + Rse;//we consider that all doors are not double or triple layered....
                double thermo = 1 / denominator;
                WallsThermalT.Add(thermo);
            ...
            OutputPorts[0].Data = WallsThermalT;
        }
        ...
        {
            double thermo_sum = 0;
            foreach (var thermo in WallsThermalT)
            {
                thermo_sum += thermo;
            }
